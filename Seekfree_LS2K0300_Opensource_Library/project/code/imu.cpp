...
float             gyro_drift_x = 5.67f;                                     // 陀螺仪 X 轴零漂
float             gyro_drift_y = -9.15f;                                    // 陀螺仪 Y 轴零漂
float             gyro_drift_z = -7.51f;                                     // 陀螺仪 Z 轴零漂
...
// 中断中更新的最新 IMU 原始数据
int16 imu_raw_acc_x = 0;
int16 imu_raw_acc_y = 0;
int16 imu_raw_acc_z = 0;
int16 imu_raw_gyro_x = 0;
int16 imu_raw_gyro_y = 0;
int16 imu_raw_gyro_z = 0;
...
extern int16      gyroscopeOffset[3] = {0};
...
void gyro_offset_get(void)
{
    if (gyro_calibrate_flag)  
    {
        ...
        {
            if (fabsf((float)imu963ra_gyro_x) < GYRO_STATIC_THRESHOLD
             && fabsf((float)imu963ra_gyro_y) < GYRO_STATIC_THRESHOLD
             && fabsf((float)imu963ra_gyro_z) < GYRO_STATIC_THRESHOLD)
            {
        ...
        gyro_x_offset_sum += (float)imu963ra_gyro_x;
        gyro_y_offset_sum += (float)imu963ra_gyro_y;
        gyro_z_offset_sum += (float)imu963ra_gyro_z;
        gyro_offset_count++;
...
void ICM_getValues(void)
{
    ...
    icm_data_t.acc_x = (-imu963ra_acc_x) * IMU_ALPHA + icm_data_t.acc_x * (1.0f - IMU_ALPHA);
    icm_data_t.acc_y = ( imu963ra_acc_y) * IMU_ALPHA + icm_data_t.acc_y * (1.0f - IMU_ALPHA);
    icm_data_t.acc_z = (-imu963ra_acc_z) * IMU_ALPHA + icm_data_t.acc_z * (1.0f - IMU_ALPHA);

    float gx_temp = (float)imu963ra_gyro_x - gyro_drift_x;
    float gy_temp = (float)imu963ra_gyro_y - gyro_drift_y;
    float gz_temp = (float)imu963ra_gyro_z - gyro_drift_z;
    ...
    icm_data_t.gyro_x = (-gx_temp) * PI / 180.0f;
    icm_data_t.gyro_y = ( gy_temp) * PI / 180.0f;
    icm_data_t.gyro_z = (-gz_temp) * PI / 180.0f;
}
...
void imu_update_attitude(void)
{
    ...
    // 3. 互补滤波更新四元数
    ICM_AHRSupdate(icm_data_t.gyro_x, icm_data_t.gyro_y, icm_data_t.gyro_z,
                   icm_data_t.acc_x, icm_data_t.acc_y, icm_data_t.acc_z);
    ...
    // Pitch
    icm_data_t.pitch = asinf(-2.0f * (q1 * q3 - q0 * q2)) * 57.2957f;
    // Roll
    icm_data_t.roll  = atan2f(2.0f * (q2 * q3 + q0 * q1),
                              q0*q0 - q1*q1 - q2*q2 + q3*q3) * 57.2957f;

    // Yaw（带死区滤波，移植自 ICM_getEulerianAngles）
    static float last_raw_yaw = 0.0f;
    static float accumulated_yaw_drift = 0.0f;
    static bool yaw_first_run = true;

    float raw_yaw = atan2f(2.0f * q1 * q2 + 2.0f * q0 * q3,
                           -2.0f * q2 * q2 - 2.0f * q3 * q3 + 1.0f) * 57.2957f;
    ...
    // 更新全局角度结构体
    imu_angle.pitch = icm_data_t.pitch;
    imu_angle.roll  = icm_data_t.roll;
    imu_angle.yaw   = icm_data_t.yaw;
}
